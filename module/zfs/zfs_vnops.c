...
zfs_write(struct inode *ip, uio_t *uio, int ioflag, cred_t *cr)
{
	...
	if (zsb->z_os->os_compress >= ZIO_COMPRESS_QOS_10 && zsb->z_os->os_compress <= ZIO_COMPRESS_QOS_1000) {
		uint64_t compress;

		dsl_prop_get_dd(zsb->z_os->os_dsl_dataset->ds_dir, zfs_prop_to_name(ZFS_PROP_COMPRESSION),
				8, 1, &compress, NULL, B_FALSE, &qos_dd);

		mutex_enter(&dmu_objset_ds(zsb->z_os)->ds_dir->dd_lock);
		dmu_objset_ds(zsb->z_os)->ds_dir->dd_inherit_parent = qos_dd;
		mutex_exit(&dmu_objset_ds(zsb->z_os)->ds_dir->dd_lock);
	}
	...
	while (n > 0) {
		...
		nbytes = MIN(n, max_blksz - P2PHASE(woff, max_blksz));
			...
			if (qos_dd) {
				...
				uint64_t delay = ((size * calc) / (through_MBps));
				if (delay > dur) {
