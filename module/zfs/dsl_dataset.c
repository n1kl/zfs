...
static int
dsl_dataset_actv_lz4fast_compress_check(void *arg, dmu_tx_t *tx)
{
	...
	if (!spa_feature_is_enabled(dp->dp_spa, SPA_FEATURE_LZ4FAST_COMPRESS)) {
		dsl_dataset_rele(ds, FTAG);
		return (SET_ERROR(ENOTSUP));
	}
...
static void
dsl_dataset_actv_lz4fast_compress_sync(void *arg, dmu_tx_t *tx)
{
	char *ddname = (char *)arg;
	dsl_pool_t *dp = dmu_tx_pool(tx);
	dsl_dataset_t *ds;
...
int
dsl_dataset_activate_lz4fast_compress(const char *ddname)
{
	int error;

	error = dsl_sync_task(ddname, dsl_dataset_actv_lz4fast_compress_check,
...
dsl_dataset_actv_compress_auto_check(void *arg, dmu_tx_t *tx)
{
	...
	if (!spa_feature_is_enabled(dp->dp_spa, SPA_FEATURE_COMPRESS_AUTO)) {
		dsl_dataset_rele(ds, FTAG);
		return (SET_ERROR(ENOTSUP));
	}
...
static void
dsl_dataset_actv_compress_auto_sync(void *arg, dmu_tx_t *tx)
{
	char *ddname = (char *)arg;
	dsl_pool_t *dp = dmu_tx_pool(tx);
	dsl_dataset_t *ds;
	spa_feature_t f = SPA_FEATURE_COMPRESS_AUTO;
...
int
dsl_dataset_activate_compress_auto(const char *ddname)
{
	int error;

	error = dsl_sync_task(ddname, dsl_dataset_actv_compress_auto_check,
	    dsl_dataset_actv_compress_auto_sync, (void *)ddname, 0,
		ZFS_SPACE_CHECK_RESERVED);
...
static int
dsl_dataset_actv_compress_qos_check(void *arg, dmu_tx_t *tx)
{
	char *ddname = (char *)arg;
	dsl_pool_t *dp = dmu_tx_pool(tx);
	dsl_dataset_t *ds;
	...
	if (!spa_feature_is_enabled(dp->dp_spa, SPA_FEATURE_COMPRESS_QOS)) {
		dsl_dataset_rele(ds, FTAG);
		return (SET_ERROR(ENOTSUP));
	}
...
static void
dsl_dataset_actv_compress_qos_sync(void *arg, dmu_tx_t *tx)
{
	char *ddname = (char *)arg;
	dsl_pool_t *dp = dmu_tx_pool(tx);
	dsl_dataset_t *ds;
	spa_feature_t f = SPA_FEATURE_COMPRESS_QOS;
...
int
dsl_dataset_activate_compress_qos(const char *ddname)
{
	int error;

	error = dsl_sync_task(ddname, dsl_dataset_actv_compress_qos_check,
	    dsl_dataset_actv_compress_qos_sync, (void *)ddname, 0,
		ZFS_SPACE_CHECK_RESERVED);
