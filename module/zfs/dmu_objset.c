	...
	*blkptr_copy = *os->os_rootbp;
	...
	if (dmu_objset_ds(os)) {
		if (dmu_objset_ds(os)->ds_dir->dd_inherit_parent) {
			mutex_enter(&dmu_objset_ds(os)->ds_dir->dd_inherit_parent->dd_lock);
			dmu_objset_ds(os)->ds_dir->dd_inherit_parent->dd_qos_size = 0;
			dmu_objset_ds(os)->ds_dir->dd_inherit_parent->dd_qos_ts = gethrtime();
			mutex_exit(&dmu_objset_ds(os)->ds_dir->dd_inherit_parent->dd_lock);
		}
	}
