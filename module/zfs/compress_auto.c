	...
	if (pio) {
		if (zio->io_compress_auto_delay) {
			...
			uint64_t compressBps = (zio->io_lsize * trans)
			    / zio->io_compress_auto_delay;
			compress_auto_calc_avg_nozero(compressBps,
			    &pio->io_compress_auto_Bps[zio->io_compress_level],
			    n);
		}
...
size_t compress_auto(zio_t *zio, enum zio_compress *c, abd_t *src, void *dst,
	...
	uint64_t exp_queue_delay = compress_auto_min_queue_delay(rvd,
	    zio->io_lsize);
			...
			if ((zio->io_lsize * trans)
				...
				if (level < COMPRESS_AUTO_LEVELS - 1) {
					if (pio->
						...
						if ((zio->io_lsize * trans)
						    / pio->
						    io_compress_auto_Bps[level
						    + 1] <
						    exp_queue_delay) {
							level++;
						} // else stay on level
					} else if (pio->
					    io_compress_auto_exploring ==
					    B_FALSE) {
						pio->
						    io_compress_auto_exploring =
						    B_TRUE;
